package org.coursera.capstone.syman.oauth;
import java.util.ArrayList;
import java.util.List;
import java.util.concurrent.Executor;

import org.apache.commons.io.IOUtils;
import org.coursera.capstone.syman.util.LoggedInFileHandler;
...
public class SecuredRestBuilder extends RestAdapter.Builder {
	...
	private class OAuthHandler implements RequestInterceptor {
		...
		public OAuthHandler(Client client, String tokenIssuingEndpoint, String username,
				String password, String clientId, String clientSecret, Context appContext) {
			super();
			this.client = client;
		...
		public OAuthHandler(Client client, String tokenIssuingEndpoint, String oauthToken,
				String clientId, String clientSecret, Context appContext) {
			super();
			this.client = client;
			this.tokenIssuingEndpoint = tokenIssuingEndpoint;
			...
			if (!loggedIn) {
				try {
					...
					if (resp.getStatus() < 200 || resp.getStatus() > 299) {
						...
						LoggedInFileHandler.saveAccessToken(appContext, accessToken);
	...
	public SecuredRestBuilder setOauthToken(String oauthToken) {
	...
	public RestAdapter build() {
		if ((username == null || password == null) && oauthToken == null) {
		...
		if (oauthToken != null) {
			OAuthHandler hdlr = new OAuthHandler(client, loginUrl, oauthToken, clientId, clientSecret, appContext);
			setRequestInterceptor(hdlr);
		} else {
			OAuthHandler hdlr = new OAuthHandler(client, loginUrl, username, password, clientId, clientSecret, appContext);
			setRequestInterceptor(hdlr);
		}
